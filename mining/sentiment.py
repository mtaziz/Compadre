...
def analyze_sentiment(sentence, features):
    """analyze sentiments expressed about product features in sentences"""
    positive_words = open('../datasets/positive-words.txt', 'r').read().split('\n')
    negative_words = open('../datasets/negative-words.txt', 'r').read().split('\n')
    ...
    for i,w in enumerate(sentence_words):
        ...
        for f in features:
            feature_occurences = [i_ for i_,x in enumerate(sentence_words) if x == f]
            promixity_to_features.append((f, min([abs(i-o) for o in feature_occurences])))
